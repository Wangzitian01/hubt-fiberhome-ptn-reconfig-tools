...
import cn.edu.hbut.hfcrt.service.TopoService;
...
//		RingLeftTest(path); //<==
	...
	public static void RingRightTest(String path) throws IOException{
		...
		for(int i=0;i<=n;i++){
			...
			//筛选满足条件的tunnel
			List<Document> tunnelList1= ChangeTunnelService.findFilterConditionTunnel(tunnelList,list5);
...
//			//新加
//			Map<Integer,List<Integer>> map = TunnelService.findTunnelAndNeId(tunnelList1, path);
//			ReadExcel.writeListToExcel(map,"C:\\Users\\13409\\Desktop\\烽火\\tunnel变化前结果.xls");	
//			//新加
...
//	
////	@Test
//	public void PtnTestOne() throws IOException{
//		
//		for(int i=n;i<=n;i++){
//			System.out.println("当前行数是第==========================================="+i+"行=======================================");
//            List<Integer> lList = RingController.findMidLeftNeIdFromAllRingInfo(ExcelPath, rowNum);
...
//			
//			//5.根据boardId,portName,neId,查找经过该网元的所有tunnel
//			List<Document> tunnelList = PtnService.selectTunnels1(boardId1, portName1, neId1, "workPath");
//			System.out.println("-----------------初始tunnel的条数---------------------"+tunnelList.size());
//			System.out.println("初始tunnel中第一条情况"+tunnelList.get(0));
//				
//			
...
//				}
//			}
//			System.out.println("初步筛选的tunnel中情况"+tunnelList.get(0));
//			System.out.println("初步筛选的tunnel条数"+tunnelList.size());
//			System.out.println("初步筛选后目标网元集合情况"+GList);
//			System.out.println("初步筛选后目标网元集合大小"+GList.size());
//			System.out.println("初步筛选后汇聚网元集合情况"+CList);
...
//			System.out.println("第二次筛选的tunnel的条数:"+list5.size());
//			System.out.println("第二次筛选满足的tunnel的下标索引"+list5);
//			System.out.println("哈哈哈哈"+tunnelList.get(2));
//			
//			//二次筛选,目标集合以及汇聚集合
//			
//			List<Integer> CList1 = ChangeTunnelService.selectFilterConditionCListAndGListIndex(list5, CList);
...
//			
//			Document Atunnel = tunnelList1.get(0);
//			Document APath = (Document) ((ArrayList) Atunnel.get("workPath")).get(0);//获取该Tunnel中的workpath
//			List<Object> ApathsList = (ArrayList) APath.get("pathsList");
//			System.out.println("未进行业务操作之前的tunnel中第一条tunnel为"+tunnelList1.get(0));
//			System.out.println("未进行业务操作之前的tunnel中第一条tunnel中pathlist中的长度为"+ApathsList.size());
//		
...
//	
//			//删除中间网元到目标网元之间的网元==业务切换
//			ChangeTunnelService.DeleteNeFromgoalNeToConvengenceNe((int)neId1, FConvergenceNeId2, tunnelList1,"workPath");
//			
//			System.out.println("业务操作之后tunnel情况"+tunnelList1);
//			System.out.println("业务操作之后tunnel条数"+tunnelList1.size());
//			
//			int _id = (int) tunnelList1.get(0).get("_id");
//			int length = TunnelService.findNeIdLengthByPathsList(_id, "workPath");
...
//	@Test
	public  void PtnTestTotal() throws IOException{
...
//			Boolean flag = TunnelChange(tunnelList1,(int)neId1, FConvergenceNeId2,path);
//			if(flag==true){
//				continue;
//			}
			...
			System.out.println("业务操作之后tunnel情况"+tunnelList1);
			System.out.println("业务操作之后tunnel条数"+tunnelList1.size());
			Document Btunnel = tunnelList1.get(0);
			Document BPath = (Document) ((ArrayList) Btunnel.get("workPath")).get(0);//获取该Tunnel中的workpath
			List<Object> BpathsList = (ArrayList) BPath.get("pathsList");
			System.out.println("进行业务操作之后的tunnel中第一条tunnel为"+tunnelList1.get(0).get("_id"));
			System.out.println("进行业务操作之后的tunnel中第一条tunnel中pathlist中的长度为"+BpathsList.size());
